          ...
          <TableBody>
            {runs.map((r) => (
              <TableRow key={r.id} className="cursor-pointer" data-testid="run-row" data-run-id={r.id}>
                ...
                <TableCell className="text-right"><Link to={`/runs/${encodeURIComponent(r.id)}`} className="block"><span className="mono" data-testid="shipped-planned" title={r.status === 'legacy' ? 'Pre-v2 record: scenarios explored, no shipped count' : 'shipped / planned'}>{r.status === 'legacy' ? `${r.generated} explored` : `${r.shipped ?? 0}/${r.planned}`}</span></Link></TableCell>
