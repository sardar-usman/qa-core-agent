...
Pages in this release: **Projects** (one card per host with last run, tests shipped, open findings, spend this month, coverage sparkline), **Runs** (filter by project and status; status, shipped/planned, cost, flake rate, duration, source, date), and **Run Detail** at `/runs/<run-id>`: header, a scenarios table with the Critic verdict, repair status, replay and stability outcomes and shipped yes/no exactly as stored, findings under "Product behavior to review" (never scenario rows), artifact links for the files present in the run folder, and the stored events timeline (`events.jsonl`, written next to the report by every surface). A missing run-report is a loud 404 naming the path. Every number comes from `data/qa-core.sqlite`, an index the gateway rebuilds from `output/` on every start and refreshes after every run. Files are truth: delete the database and it is rebuilt exactly. `POST /api/reindex` (or the header button) rebuilds it on demand. Runs from before the per-run layout, which survive only as summaries in the gateway's record store (`.qa-core/sites/<host>.json`), are imported as "summary only (pre-v2)" rows with the scenarios they explored (not shipped), cost and duration, and no report or zip; project cards count tests shipped from reported runs only and show legacy runs as a separate line. With `QA_CORE_GATEWAY_TOKEN` set, every `/api` route needs `Authorization: Bearer <token>` (or `?token=`), and the app takes the token from `#token=<value>` in the page URL, in memory only.
