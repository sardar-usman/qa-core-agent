...
const report = {
  ...
  stability: { iterations: 3, passed: 2, flaked: 0, flakeRate: 0, durationMs: 9000, recovered: 1, stabilizerCostUsd: 0.004, verdicts: [
    ...
    { name: 'add to cart updates the badge', iterations: 3, passes: 2, stable: true, classification: 'stable', pattern: 'PFP', relaxed: true, durationMs: 5000,
      firstFailure: { iteration: 2, failedStep: 3, stepKind: 'assert', error: 'toHaveText: expected "1"', observed: { url: 'https://www.saucedemo.com/inventory.html', target: '0', messages: [] } },
      attempts: [{ attempt: 1, kind: 'timeout_raise', change: 'timeout_raise(10000ms)', reason: 'the badge updates asynchronously', pattern: 'P-P-P', outcome: 'recovered' }] },
  ] },
...
const rendered = await page.evaluate(() => {
  ...
  return {
    ...
    verify: { replayRows: Array.from(document.querySelectorAll('[data-testid="replay-row"]')).map((r) => r.getAttribute('data-passed')), patterns: Array.from(document.querySelectorAll('[data-testid="stability-pattern"]')).map((p) => p.textContent), recovered: document.querySelector('[data-testid="stability-recovered"]')?.textContent, stabilizerCost: document.querySelector('[data-testid="stabilizer-cost"]')?.textContent, attempts: document.querySelectorAll('[data-testid="stability-attempt"]').length, attemptText: document.querySelector('[data-testid="stability-attempt"]')?.textContent ?? '', failureText: document.querySelector('[data-testid="stability-failure"]')?.textContent ?? '', attemptsLine: document.querySelector('[data-testid="stabilizer-attempts"]')?.textContent ?? '', warning: document.querySelector('[data-testid="stabilizer-warning"]')?.textContent ?? '' },
...
if (d.status === 200 && d.body.legacy === false) {
  ...
  check('SV2. page: the Verify panel shows the recorded attempt with its change and outcome, the observed text at the failing target, an attempts line with a count (not "none recorded"), and no warning',
    rendered.verify.attempts === 1 && /timeout_raise\(10000ms\), recovered \(P-P-P\)/.test(rendered.verify.attemptText) && /saw: 0/.test(rendered.verify.failureText) && /1 across 1 scenario/.test(rendered.verify.attemptsLine) && !/none recorded/.test(rendered.verify.attemptsLine) && rendered.verify.warning === '', JSON.stringify(rendered.verify));
